...
@Component({
  selector: 'app-agasajos',
  templateUrl: './agasajos.component.html',
  styleUrls: ['./agasajos.component.css']
})
export class AgasajosComponent implements OnInit {
  // Formulario
      ...
      response => {
        ...
        if (response.status === 'success') {
          ...
          this.listaNoticias.forEach((element, index) => {
            // console.log(element);
            if (element.estado === 1 && element.estado_noticias === 'A') {
    ...
    this.acercaServices.indexAcerca().subscribe(
      response => {
        if (response.status === "success") {
          ...
          this.imagenWeb = this.listaAcerca.img_agasajo;
